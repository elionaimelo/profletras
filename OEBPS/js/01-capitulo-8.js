var surdas = [];
var sonoras = [];
var surdasCorretas = ['panela', 'chifre', 'rua', 'tapete', 'sapeca'];
var sonorasCorretas = ['moleza', 'lataria', 'lhama', 'nadador', 'verdade']
var qtdPalavrasEncontradas = 0;
var ultimaPalavraSelecionada = '';
...
function addPalavra(objeto) {
  if ($(objeto).attr('id') === 'surdas') {
    surdas.push(ultimaPalavraSelecionada);
  } else {
    sonoras.push(ultimaPalavraSelecionada);
  }
  qtdPalavrasEncontradas++;
  $('.' + ultimaPalavraSelecionada).addClass('wordFound');
  $('#puzzle, #words').removeClass('desabilitado');
  $('#opcoes').hide();
  verificarResposta();
}

function verificarResposta() {
  if (qtdPalavrasEncontradas === (surdasCorretas.length + sonorasCorretas.length)) {
    surdas.sort();
    surdasCorretas.sort();
    sonoras.sort();
    ...
    $.each(surdas, function(chave, valor) {
      if (surdas[chave] !== surdasCorretas[chave]) {
        ...
        console.log('surda:' + surdas[chave]);
    ...
    var classe = 'complete-wrong';
    var mensagem = 'Tente outra vez.';

    if (respostaCorreta) {
      classe = 'complete-right';
      mensagem = 'Parabéns, você acertou!';
    }

    $('#mensagem').show();
    $('#mensagem h2').text(mensagem);
    $('.puzzleSquare').addClass(classe);
  }
}
    ...
    */
    var drawWords = function (el, words) {
      ...
      for (var i = 0, len = words.length; i < len; i++) {
        var word = words[i];
        output += '<li class="word ' + word + '">' + word + '</li>';
      }
      output += '</ul>';
    ...
    var endTurn = function () {
      ...
      for (var i = 0, len = wordList.length; i < len; i++) {
        ...
        if (wordList[i] === curWord) {
          ultimaPalavraSelecionada = curWord;
          $('#palavraSelecionada').text(curWord);
          $('#opcoes').show();

          $('#puzzle').addClass('desabilitado');
          $('#words').addClass('desabilitado');
